...
- [ ] Task 10: 实现小说列表与详情模块
  - [ ] SubTask 10.1: `/novels` GET 渲染小说列表页（支持按分类/标签筛选，支持按评分/最后阅读时间/书籍字数排序，排序分为正序与反序，筛选与排序可合并）
...
- [ ] Task 17: 实现主题切换功能
  - [ ] SubTask 17.1: 创建 `static/css/theme.css` 定义白天/黑夜主题变量与样式覆盖
  - [ ] SubTask 17.2: 创建 `static/js/theme.js` 实现主题切换逻辑（读取/保存 localStorage、切换 body class、更新图标）
  - [ ] SubTask 17.3: 在 `templates/base.html` 右上角添加主题切换按钮（月亮/太阳图标），所有页面继承
  - [ ] SubTask 17.4: 在登录页（`templates/login.html`）独立添加主题切换按钮，确保未登录也可使用

- [ ] Task 18: 验证与测试
  - [ ] SubTask 18.1: 启动应用并验证首页跳转到登录页
  - [ ] SubTask 18.2: 验证登录、登出流程
  - [ ] SubTask 18.3: 验证章节规则列表页（8 大类默认规则）、新增/编辑/启用禁用/删除/恢复默认规则
  - [ ] SubTask 18.4: 验证导入 4 步向导：Step 1 上传转码、Step 2 选择章节规则、Step 3 预览删除章节、Step 4 确认保存（标题默认为文件名，作者可为空）
  - [ ] SubTask 18.5: 验证标签管理（新增、编辑、删除）与小说标签关联（添加、移除、按标签筛选）
  - [ ] SubTask 18.6: 验证收藏功能（收藏、取消收藏、收藏列表）
  - [ ] SubTask 18.7: 验证评分功能（首次评分、修改评分、平均评分显示）
  - [ ] SubTask 18.8: 验证阅读、进度记录、书签功能
  - [ ] SubTask 18.9: 验证在线编辑章节并查看生效
  - [ ] SubTask 18.10: 验证导出 TXT 文件
  - [ ] SubTask 18.11: 验证全文搜索结果
  - [ ] SubTask 18.12: 验证 Flask-Admin 后台需登录可访问
  - [ ] SubTask 18.13: 验证手机视口下页面布局正常
  - [ ] SubTask 18.14: 验证主题切换：未登录可切换、切换后刷新页面保持主题、白天/黑夜配色正确
