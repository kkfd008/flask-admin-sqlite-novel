...
- [ ] Task 7: 实现小说列表与详情模块
  - [ ] SubTask 7.1: `/novels` GET 渲染小说列表页（支持按分类筛选）
  - [ ] SubTask 7.2: `/novels/<id>` GET 渲染小说详情页（章节目录、阅读进度、操作按钮）
  - [ ] SubTask 7.3: `/novels/<id>/delete` POST 删除小说及其章节、进度、书签

- [ ] Task 8: 实现小说阅读模块
  - [ ] SubTask 8.1: `/novels/<id>/read/<chapter_id>` GET 渲染阅读页（章节正文 + 上下章 + 书签按钮）
  - [ ] SubTask 8.2: 上/下一章切换，更新阅读进度
  - [ ] SubTask 8.3: `/api/progress` POST 保存阅读进度（chapter_id + scroll_position）
  - [ ] SubTask 8.4: `/api/bookmark` POST 添加书签，`/api/bookmark/<id>/delete` 删除书签
  - [ ] SubTask 8.5: 书签列表与跳转

- [ ] Task 9: 实现在线内容修改模块
  - [ ] SubTask 9.1: `/novels/<id>/chapter/<chapter_id>/edit` GET 渲染编辑页
  - [ ] SubTask 9.2: `/novels/<id>/chapter/<chapter_id>/edit` POST 保存标题与正文修改

- [ ] Task 10: 实现小说导出模块
  - [ ] SubTask 10.1: `/novels/<id>/export` GET 返回 `<小说标题>.txt` 文件流（章节按序拼接，空行分隔）

- [ ] Task 11: 实现全站全文搜索模块
  - [ ] SubTask 11.1: `/search` GET 接收 `q` 参数，对 `Chapter.content` 做 LIKE 查询
  - [ ] SubTask 11.2: 渲染搜索结果页（小说标题、章节标题、匹配摘要、跳转链接）

- [ ] Task 12: 集成 Flask-Admin 后台
  - [ ] SubTask 12.1: 在 `app/__init__.py` 中初始化 Admin（template_mode='bootstrap4'）
  - [ ] SubTask 12.2: 为 Category、Novel、Chapter、ReadingProgress、Bookmark、User、ChapterRule 注册 ModelView
  - [ ] SubTask 12.3: 为 Admin 视图增加 `is_accessible` 鉴权，未登录重定向到 `/login`

- [ ] Task 13: 完善响应式 UI 模板
  - [ ] SubTask 13.1: `templates/base.html` 引入 Layui，使用 Layui 响应式栅格布局
  - [ ] SubTask 13.2: 登录页、列表页、详情页、阅读页、编辑页、搜索页、规则管理页均适配手机（<992px）
  - [ ] SubTask 13.3: 阅读页字体大小、按钮在手机下可正常操作

- [ ] Task 14: 验证与测试
  - [ ] SubTask 14.1: 启动应用并验证首页跳转到登录页
  - [ ] SubTask 14.2: 验证登录、登出流程
  - [ ] SubTask 14.3: 验证规则列表页、新增/编辑/启用禁用/删除/恢复默认规则
  - [ ] SubTask 14.4: 验证导入一本含章节的 TXT 小说（使用规则与自定义正则两种方式）并查看章节列表
  - [ ] SubTask 14.5: 验证阅读、进度记录、书签功能
  - [ ] SubTask 14.6: 验证在线编辑章节并查看生效
  - [ ] SubTask 14.7: 验证导出 TXT 文件
  - [ ] SubTask 14.8: 验证全文搜索结果
  - [ ] SubTask 14.9: 验证 Flask-Admin 后台需登录可访问
  - [ ] SubTask 14.10: 验证手机视口下页面布局正常
...
- Task 8 依赖 Task 2、Task 3、Task 7
- Task 9 依赖 Task 2、Task 3、Task 7
- Task 10 依赖 Task 2、Task 3、Task 7
- Task 11 依赖 Task 2、Task 3、Task 7
- Task 12 依赖 Task 1、Task 2、Task 3
- Task 13 依赖 Task 1（可与 Task 3-11 并行进行模板开发）
